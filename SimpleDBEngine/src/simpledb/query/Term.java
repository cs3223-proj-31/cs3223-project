...
public class Term {
   ...
   public Term(Expression lhs, Expression rhs) {
      this(lhs, rhs, new Operator("="));
   }

   /**
    * Create a new term that compares two expressions
    * based on a given custom operator
    * @param lhs  the LHS expression
    * @param rhs  the RHS expression
    ...
    * fulfill relationship specified by operator
    * with respect to the specified scan.
    * @param s the scan
    * @return true if both expressions fulfill relationship specified by operator in the scan
    */
   public boolean isSatisfied(Scan s) {
      Constant lhsval = lhs.evaluate(s);
      Constant rhsval = rhs.evaluate(s);
      return op.Compare(lhsval, rhsval);
   }
